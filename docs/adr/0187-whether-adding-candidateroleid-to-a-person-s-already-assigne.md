# 0187. Whether adding candidateRoleId to a person's already-assigned roleIds
...
This rationale is applied at 4 call site(s) within `src/utils/scheduler.ts`. No external review/research document is cited for this decision — it was a file-local judgment call.

Whether adding `candidateRoleId` to a person's already-assigned roleIds for a given date (`assignedRoleIdsThisDate`) keeps the resulting combo legal (D-10/D-12).
