# Phase 18 Research: Song Lyric Slides and Editor

**Depth:** Deep research — CCLI paste parsing is novel (no prior art in codebase), the unified slide data model is new, the performance order builder requires drag-and-drop UX, and auto-save infrastructure must be adapted from a different context (services → songs).

> Ported faithfully from gsdpi slice S01 research (`.gsd/milestones/M001/slices/S01/S01-RESEARCH.md`). "Slice" terminology has been mapped to "phase" where it reads naturally; technical content is unchanged.

## Summary

This phase delivers the foundational slide system: CCLI SongSelect paste → auto-parsed lyric sections → slide editor with performance order → copyright compliance → auto-save → light versioning. No slide types, lyric storage, or slide editor exist yet. The Song type has metadata (title, ccliNumber, author, arrangements) but zero lyric content. Everything in this phase is net-new.
...
The unified slide data model (R019) maps to a `Slide` type with a `contentKind` discriminator field. For this phase, only `contentKind: 'lyric'` exists. Phase 19 adds `'scripture'`, Phase 21 adds `'imported'`, etc.

### Performance Order: Separate from Lyrics

The performance order (R003) is an ordered array of section references: `['verse-1', 'chorus', 'verse-2', 'chorus', 'bridge', 'chorus']`. This lives on the Song doc itself (not in the lyrics subcollection) because it's lightweight and needs to be accessible without loading full lyric text — services reference it to build slideshows in a later phase.
...
3. **Slide preview fidelity** — How closely should the editor preview match the eventual presentation mode (later phase)? For this phase, a simple card-style preview per section is sufficient. Full-screen presentation rendering is a later phase's scope.
