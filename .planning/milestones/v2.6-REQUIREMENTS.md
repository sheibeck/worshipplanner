# Archived: v2.6 Per-Org Bible API Toggle & Manual Fallback

**Archived:** 2026-08-31
**Status:** ✅ SHIPPED & DEPLOYED TO PRODUCTION 2026-08-31 (tag `v2.6`)
...
All 7 requirements validated. R299 was delivered per-spec in Phase 103 and then **refined by owner
direction** before ship: the standalone paste-the-passage textarea and the off-state explanatory message
were removed. Plain scripture slides are reference-only (BibleGateway deep-link only) when the API is off;
congregational readings are composed directly in the pre-existing bottom reading/format textarea, with the
AI split still operating on that text under the independent AI gate. No other requirement was adjusted or
dropped. See [v2.6-MILESTONE-AUDIT.md](../v2.6-MILESTONE-AUDIT.md) for independent verification detail.
