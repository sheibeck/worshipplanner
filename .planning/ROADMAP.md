...
**Plans**: 3 plans (all wave 1 — no file overlap, fully parallel)

- [ ] 80-01-PLAN.md — Rules hardening: `inviteLookup` create gated to the target-org editor (R232) + org `createdBy` immutability on update (R233); emulator ALLOW/DENY + acceptance/edit regressions; ships UNDEPLOYED with the `firebase deploy --only firestore:rules` hand-over
- [ ] 80-02-PLAN.md — `deleteService` revokes all three share artifacts — `shareTokens` (query, multiple) / `serviceShareLinks` / `serviceShares`, existence-guarded, never-shared no-throw (R234, client-only)
