...
overrides_applied: 0
human_verification:
  - test: "After a real `firebase deploy --only hosting`, in a browser that previously had the app cached and WITHOUT a manual cache-clear: (a) load the production ROOT url `/` and a deep link (e.g. `/services/<id>`); (b) inspect a hashed asset request under `/assets/`."
    expected: "(a) The browser re-fetches `index.html` for both `/` and the deep link (network tab shows a fresh document request, not `(disk cache)`/`(memory cache)`) and the newly deployed bundle loads immediately. (b) The `/assets/*` response still carries `Cache-Control: public, max-age=31536000, immutable`, NOT no-cache."
    why_human: "R109 is deploy-gated by the standing v1.5 NO-DEPLOYS grant — no `firebase deploy` was run during this phase. NOTE: this item's config was strengthened AFTER initial verification — code review (WR-01) found the original `/index.html`-only header never applied to `/` or SPA deep links (Firebase matches header globs against the incoming pre-rewrite path), so it was widened to `**` no-cache + `/assets/**` immutable per owner decision 2026-08-10 (commit dd9adc8; behavioral guard test now 3/3). Part (b) is the backstop: the config's correctness depends on Firebase's last-match-wins header precedence (confirmed from community sources; official docs silent) — if assets come back no-cache in production, reorder so `/assets/**` wins. See PENDING-VERIFICATION item 50.1."
