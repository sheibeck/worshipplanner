# Phase 20: Service Sections and Slide Auto-Assembly — Research

**Researched:** 2026-07-24
**Domain:** Additive data-model evolution + pure-function derivation over an existing Vue 3 + Pinia + Firestore worship-service model
**Confidence:** HIGH (all findings are direct reads of this codebase; no external ecosystem research was needed)

> This document is a faithful port of the gsdpi slice **S03-RESEARCH.md** into the gsd-core Phase 20 structure. It preserves the original research findings. Requirement IDs (R005/R006/R007/R018) and decision IDs (D005) originate in the M001 milestone context.

## Summary

Phase 20 introduces formalized service sections (Pre-Service, Worship, Message, Sending) and a slideshow auto-assembly engine that collects slides from songs (Phase 18) and scripture readings (Phase 19) based on the service order. This is deep research — the current service model is a flat `slots[]` array with no section grouping, and there is no slide assembly infrastructure at all. The phase must add a section layer to the data model without breaking existing services, build a pure-function assembly engine, and wire it into the ServiceEditorView UI.
...
- `src/components/SlideshowPreview.vue` — **New file.** Renders the assembled slideshow as a scrollable slide list with section dividers. Not the full-screen presentation (that's Phase 23 / S06) — just an inline preview showing what the assembled slideshow looks like.
...
- **useAutoSave composable** (Phase 18 pattern): The assembled slideshow is read-only derived state — it does not need auto-save. But any new editable state (section assignments) should use the existing 800ms debounce auto-save via `updateService()`.
- **Create-on-first-fetch** (Phase 19 pattern): Not applicable here — assembly is derived, not persisted separately.
...
- **PRAYER/MESSAGE/HYMN slots**: Generate a simple `TextSlide` with the slot label as content (placeholder for Phase 21 / future content).
...
**Forward intelligence from Phase 18**: performanceOrder is stored on the Song doc (not lyrics subcollection) so it persists across lyric version changes. The assembly engine must read performanceOrder from the Song, not from SongLyrics. SortableJS DOM-revert pattern must be preserved in any slot list UI changes.

**Phase 19 (S02) provides**:
...
**Forward intelligence from Phase 19**: `scriptureReadingId` on ScriptureSlot is the key link for assembly — the engine looks up the ScriptureReading by this ID to get pre-split slides. readingMode determines whether to use normal or congregational slide variants. Create-on-first-fetch means a ScriptureSlot might not have a `scriptureReadingId` yet (user hasn't fetched ESV text) — assembly must handle this as "empty scripture slot."
...
*Ported from gsdpi slice S03-RESEARCH.md on 2026-07-24*
