...
- **D-08:** Replace the two "Show only tag" / "Hide tag" dropdowns (`filterTagInclude`/`filterTagExclude`) with a **single combined tag control** sourcing the de-duplicated union of `teamTags ∪ themes ∪ tags`, with per-tag Show/Hide controls and a Clear action. The three `Song` fields (`teamTags`, `themes`, `tags`) stay separate in the data model — no merge — only the UI/filter surface is unified. (Amended per 12-UAT test 3 Option A, 2026-07-02: the original "user tags only" scope was widened to the teamTags ∪ themes ∪ tags union in plans 12-06/12-07; the standalone teamTags "All tags" select is absorbed into this control.) (Superseded 2026-07-02 live-UAT: the single global Hide toggle was replaced with two independent per-tag sets — `tagFilterInclude` and `tagFilterExclude` — so a user can Show some tags and Hide others simultaneously, e.g. show "Orchestra" while hiding "Christmas".)
- **D-09:** The **include set** combines with **OR** — a song shows if it carries ANY included tag (when the include set is non-empty). Including more tags broadens results.
- **D-10:** The **exclude set always wins**: a song carrying ANY excluded tag is dropped, even if it also carries an included tag. Evaluation order: (1) drop if any excluded tag matches; (2) else, if the include set is non-empty, keep only if any included tag matches; (3) else keep. Matching is across the union of `teamTags ∪ themes ∪ tags`. A tag can never be in both sets at once — toggling Show on a tag removes it from Hide and vice versa.
- **D-11:** A **"Clear"** action empties both the include and exclude sets (returns the tag filter to neutral). Clear is scoped to the tag filter only — it does NOT touch the search box or the VW-type/Key dropdowns.
