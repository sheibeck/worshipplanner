...
# ✔ SHIPPED MILESTONE — v2.6 Per-Org Bible API Toggle & Manual Fallback (shipped & deployed to production 2026-08-31, archived 2026-08-31)

**Status:** v2.6 milestone complete.

> ✅ **Deployed 2026-08-31** (tag `v2.6`). Rules + `functions:api`/`setOrgBibleEnabled`/`listOrganizations`
> + hosting deployed to production. Audit PASSED (7/7 requirements, 5/5 integration seams WIRED); two
> code-review rounds caught and fixed a Planning-Center-export gate bypass and two fallback data-loss bugs
> before ship. Closed on owner acceptance with human/visual UAT batched and deferred
> (`.planning/v2.6-DEFERRED-UAT.md`) per explicit owner instruction for this autonomous run. **Standing
> owner follow-up:** each production org (incl. Berean) must be explicitly enabled for the Bible API via
> the Owner Console, since the default is OFF.

**Goal:** Put Bible API access behind a per-organization on/off switch controlled from the Owner Console,
and when it is off give that org a zero-cost manual path so scripture and congregational-reading features
always work without passing pay-only, non-commercial API costs to users. Requirements R295–R301, archived
to `milestones/v2.6-REQUIREMENTS.md`.

(v2.5 ended at Phase 100); this milestone was **Phases 101–103**.

**Delivered (3 phases, 6 plans):**
...
- **Phase 103 — Manual Fallback When Off (R298, R299, R300):** "Open in BibleGateway" deep-link (any
  ...
  composed in the existing reading textarea.

**Coverage:** 7/7 v2.6 requirements mapped and validated, each to exactly one phase; no orphans, no
duplicates.

Full record: [milestones/v2.6-ROADMAP.md](milestones/v2.6-ROADMAP.md) ·
...
- [Phase ?]: Reused existing SERVICE_SHARE_BASE_URL param for the volunteer magic-link continue-URL (+ /volunteer/verify) rather than adding a new Cloud Functions param
- [Phase ?]: actionCodeSettings computed once per message (not per recipient) since only the target email varies per generateSignInWithEmailLink call
- [Phase ?]: Router isVolunteerRoute RouteMeta exemption (Pitfall 1): a zero-membership magic-link volunteer no longer bounces to the empty /select-church picker
...
- Phase 142 added (2026-09-19): Slides Tab panel UX from Claude Design — rework slide banner/audio/background-image panel to match the owner's Claude Design 'Slides Tab' mockup (import via claude_design MCP; files `Slides Tab.dc.html` + `support.js`)
...
|---|-------------|------|--------|-----------|
| 260919-mvw | **Vamp audio resolves from the live vamp doc** (owner local repro 2026-09-19: vamp assigned to the Scripture group before its MP3 was attached → `bedVampId` set, `bedAudioUrl` absent → "♪ Vamp: Key of C" but silent). `slideshowAssembler.ts` gains optional `AssemblyInputs.vampsById`; entry `vampId` / group `bedVampId` now resolve to the vamp's CURRENT `attachment.downloadUrl`, with the stored `audioUrl`/`bedAudioUrl` snapshot only as the deleted-vamp fallback (R440 preserved). Also covers replace-after-assign (vamps.ts deletes the old Storage object on replace). `useSlideshowAssembly.ts` subscribes the vamp store (guarded on the store's `orgId`, never torn down per-instance) and passes `vampsById`; `vamps.ts` listener now swallows permission-denied (volunteer Rehearse view). 226/226 + 17/17 targeted, type-check clean. Supersedes 141-CONTEXT's "no propagation on replace" line. | 2026-09-19 | 363ebd21…826d09d7 (deployed) | [260919-mvw-vamp-audio-resolves-from-the-live-vamp-d](./quick/260919-mvw-vamp-audio-resolves-from-the-live-vamp-d/) |
| 260919-k9j | **Run the Service fixes** (owner, 2026-09-19): (1) the On-screen LIVE tag/pane ring in `RunPreviewPair.vue` is on-air **red** like the header pill (was still green); (2) audio **defaults On** for every Run session — a `watch(live)` rising-edge in `useRunControl.ts` arms audio and calls `tryPlayAudio()` after `nextTick()`, so an assigned vamp with an MP3 (e.g. a Scripture slide) plays as soon as it goes live; the header toggle now reads **Audio: Off / Audio: On** (identifiers `audioArmed`/`toggleAudioArmed` unchanged; blocked-autoplay banner still the fallback); (3) the filmstrip **End of item** cap is a real button that emits `next-item` → `goByItem(1)` (the ArrowDown path), disabled at end of service. 7 Run test files 119/119, type-check clean. Owner's live report "my vamp isn't playing" was (2). | 2026-09-19 | 40c48537…b68fe07c | [260919-k9j-run-the-service-fixes-on-screen-live-dot](./quick/260919-k9j-run-the-service-fixes-on-screen-live-dot/) |
| 260918-pms | **Group vamp UX correction** (owner, same-day UAT of 260918-nm2): the group-panel affordance is now a real button reading **"+ Add a vamp for this group"** (class string copied verbatim from the "＋ Add music for this group" neighbour), and it — plus **Change** in the assigned state — opens a new `VampPickerSlideOver` (the app's standard slide-over shell from RoleSlideOver: Teleport, backdrop, 480px panel, header + ×, Escape/backdrop/×/Cancel close) hosting `VampPicker` in a new opt-in `fill` mode (list grows to the panel; default unchanged so the per-slide drawer is untouched). Props/emits unchanged → SlideGrid/EditSlideDrawer untouched. 6 TDD commits (4 executor + 2 orchestrator for `fill`); 236/236 on the 4 touched suites; type-check clean. Pre-existing glyph mismatch noted, not changed: music label uses fullwidth ＋, background/vamp use ASCII +. **⚠ NOT DEPLOYED yet** — ships with the next `firebase deploy --only hosting`. | 2026-09-18 | ee5e2cef, 40ae6cf4, 5cb3f4ee, a912e019, b69cc41a, 2d80adfc | [260918-pms](.planning/quick/260918-pms-choose-a-vamp-as-a-real-button-opening-a/) |
| 260918-nm2 | **Group-level vamp bed** (owner UAT request 2026-09-18). The Slides-tab group media panel gains **Choose a vamp** beside "＋ Add music for this group": assigning writes the vamp MP3 into `SlideGroup.bedAudioUrl` + new display-only `bedVampId`/`bedVampLabel` (`setGroupBedMedia`; Clear/remove `deleteField`s all three; a URL-only upload evicts the vamp fields so the label can never go stale). Assigned state shows `Vamp: {label}` · plays across all N slides · Change/Clear (+ "(no longer in library)"), inline `VampPicker`. **Vamp-sourced beds loop** — explicit, narrow exception to D-04 (uploaded beds still never loop) in `resolveEntryMedia` + the synthetic scripture-ref slide. Run: `useRunControl.vampLabelFor` falls back to the group bed (entry vampId wins; unresolved groupSlideId → no badge); `SlidePlanRail` shows `Vamp: {label}` for a vamp bed. R440: `countAssignments` also matches `bedVampId` so `deleteVamp` keeps the MP3 for group-assigned vamps. 6 TDD commits, 54 new tests, 415/415 on the 8 touched files, type-check clean. **DEPLOYED to prod 2026-09-18** (owner-confirmed, hosting only, bundle index-6wGMF5op.js). | 2026-09-18 | baaa138e, 6c865756, ee4fe910, b93f99af, 65aa5e0a, f2676601 | [260918-nm2](.planning/quick/260918-nm2-group-level-vamp-bed-choose-a-vamp-on-th/) |
...
| 260819-one-way-serve-with | "Must serve with" pairings changed from **bidirectional to one-way (directional)**. A person's `pairedWith` is now the list of people THEY must serve with; the scheduler pulls those partners in when that person is scheduled ("Nolan needs Tim" no longer implies "Tim needs Nolan"). `setPersonAvailability` (`quarters.ts`) dropped the symmetric added/removed diff and ALL reciprocal partner writes — a save now touches only `personQuarterData.{id}`, so removing a partner edits only this person's list. **Scheduler unchanged**: `propagatePairing` already follows each person's own `pairedWith`, so one-way falls out for free. UI cues flipped: drawer label bidirectional→one-way, roster marker `↔`→`→`; `pairedWith` type comment updated; 4 reciprocal-write `quarters.test.ts` cases rewritten to assert the partner is NOT touched. CSV import (`applyCsvToQuarter`) left untouched per owner (dead feature). Targeted suites 87/87, type-check clean, app suite green at 2-file baseline. ⚠ **Owner data step (LIVE, do AFTER deploy):** split the two existing two-way pairs via scoped UI edits — open **Tim**→remove **Nolan**→Save, open **Gabriel**→remove **Lilly**→Save. Do NOT do these before deploy (old reciprocal code would strip the kept side too). ✅ **Deployed to production (hosting) 2026-08-19** — one-way removal live. ✅ Owner completed both data splits in the UI (Nolan→Tim, Lilly→Gabriel); loop fully closed. | 2026-08-19 | c64502bd | [260819-one-way-serve-with](.planning/milestones/quick-archive/260819-one-way-serve-with/) |
| 260823-onboard-admin-email | Onboarding a new church now emails the assigned first admin via the existing Resend system (best-effort, after the transaction commits — never fails onboarding; returns `emailSent`). New reusable `functions/src/adminEmail.ts` (`sendAdminOnboardingEmail`, added/invited copy, From = `"<Church>" <sender.fromAddress>`, header-injection sanitized) + `functions/src/params.ts` (moved `RESEND_API_KEY`/`SERVICE_SHARE_BASE_URL`/`bareEmailAddress` out of index.ts to break the circular import); `onboardOrganization` gained `{ secrets: [RESEND_API_KEY] }`. `assignOrgAdmin` intentionally unchanged (owner-scoped to onboarding). Functions build exit 0; functions 553/553; type-check clean; app suite baseline. ⚠ HAND-OVER: `firebase deploy --only functions:onboardOrganization`. Delivery to arbitrary admins still awaits a verified domain (backlog 999.6). | 2026-08-23 | e8422944 | [260823-onboard-admin-email](.planning/milestones/quick-archive/260823-onboard-admin-email/) |
| 260823-org-row-buttons | Organizations-tab row actions restyled into the app's standard button family (bg-gray-800 secondary / bg-indigo-600 primary / bg-red-600 destructive, compact `rounded-md px-3 py-1.5 text-xs`), laid out side-by-side on ≥ sm and stacked only on mobile (`flex-col sm:flex-row sm:flex-wrap`); the Delete button is now RENDERED only for an already-deactivated org (`v-if org.active === false`) instead of shown-but-disabled; per-row feedback moved below the button row. Client-only, no deploy. OrganizationsTab.test.ts 40/40 (Delete-gating test updated to absent-for-active/present-for-deactivated); type-check clean. | 2026-08-23 | b2dc4788 | [260823-org-row-buttons](.planning/milestones/quick-archive/260823-org-row-buttons/) |
| 260822-login-church-picker | Signing in no longer auto-creates an org (removed the `ensureUserDocument` auto-create branch that produced duplicate "<name>'s Church" orgs on a fresh sign-in) — orgs are created only by a super-admin via `onboardOrganization`, and login now only *joins* an org via a pending invite. Added a multi-church login picker (`SelectChurchView`, `/select-church`) with a router org-selection gate: a user in >1 church picks one (choice remembered per session; switch = log out/in), a user in none sees an empty "ask your administrator" state, org-scoped routes redirect accordingly (owner-console exempt). Found in v2.0 emulator UAT; the reported "Zome Church 0 members" was a test-data email typo (`jouctajaxx@` vs `jouctasjaxx@`), not a bug — invites match exact email. Client-only, no deploy. type-check clean; targeted suites 87/87; app suite green at the documented 2-file baseline. | 2026-08-22 | 11464520 | [260822-login-church-picker](.planning/milestones/quick-archive/260822-login-church-picker/) |
| 260823-switch-church-cache | Switching churches (own church → Enter another church as super-admin, and the multi-church picker) briefly showed the previous church's data. Root cause: each org-scoped store's `subscribe(orgId)` re-points its Firestore listener but keeps the old `.value` until the new snapshot arrives, and Vue Router mounts the destination view before the source unmounts — so the stale array flashes. **Fix:** new `src/stores/orgScopedStores.ts` `resetOrgScopedStores()` unsubscribes + clears state for all 10 org-scoped stores (services, songs, roster, quarters, slideGroups, scriptureSlides, importedSlides, pptxRenders, serviceMessages, songLyrics); `auth.ts` calls it (dynamic import, avoids auth↔store cycle) at every switch point BEFORE the new church loads — `enterOrgAsSuperAdmin`, `selectOrg`, `exitSuperAdminView`. Client-only. type-check clean; auth store 93/93; build exit 0. ⚠ HAND-OVER: `firebase deploy --only hosting`. | 2026-08-23 | 11064ac5 | [260823-switch-church-cache](.planning/milestones/quick-archive/260823-switch-church-cache/) |
| 260823-superadmin-nav-fix | v2.1 Phase 78 UAT finding: exiting a super-admin church view (`exitSuperAdminView`) left a broken partial nav (only "Services" + "Owner Console") because `resetOrgContext()` nulls `userRole` while "Services" was pushed unconditionally, and the super-admin's own church context was never reloaded; a churchless super-admin was routed to the empty `/select-church` picker; and there was no indicator when a super-admin was outside their own church. **Fix (owner decisions):** `exitSuperAdminView` is now async and reloads the super-admin's OWN church via `loadOrgContext` (restores own-church nav + Owner Console); "Services" nav item gated on `authStore.orgId`; new `isChurchlessSuperAdmin` / `superAdminOutsideOwnChurch` computeds; router lands a churchless super-admin on `owner-console` (gate + select-church redirect-away + post-login) instead of the empty picker; sidebar shows "Super Admin · not in a church" (no active church) and a "viewing as super-admin" tag (in another church); `AppShell.onExitSuperAdminView` awaits the async exit. Client-only. type-check clean; router/auth/AppShell 107/107; app suite at 2-file baseline; production build exit 0. ⚠ HAND-OVER: `firebase deploy --only hosting`. | 2026-08-23 | a8b37fe4 | [260823-superadmin-nav-fix](.planning/milestones/quick-archive/260823-superadmin-nav-fix/) |
| 260817-auto-share-link-on-create | Auto-generate a service's share link at creation so every service always has one — a volunteer message's `{{service_link}}` resolves server-side from the newest `shareTokens` doc, which previously only existed after a manual Share click (so an un-shared service emailed an empty link). `createService` now calls the existing `ensureShareLink` (soft-fail try/catch — a share error never fails the create); `maybeRefreshShareLink` keeps it current on later edits. Client-only; no functions/rules change. `services.test.ts` 95/95 (+2), type-check clean. Forward-looking: NEW services only; pre-existing never-shared services still need one explicit **Share** click (an edit does NOT auto-create — `maybeRefreshShareLink` never takes the create branch). | 2026-08-17 | d2cce8bb | [260817-auto-share-link-on-create](.planning/milestones/quick-archive/260817-auto-share-link-on-create/) |
| 260824-org-config-slideout | Owner Console Organizations screen reworked: per-org configuration (AI enablement, Active) moved from crowded per-row buttons into a right-side `OrgConfigDrawer.vue` slideout (mirrors `AvailabilityDrawer.vue`'s shell + `EditSlideDrawer.vue`'s Escape/focus handling) with one-way `:checked`+`@change` checkboxes (never `v-model`). Row keeps only a single "Configure" entry point plus Assign admin/Enter church/Delete. New `DeactivateOrgConfirmDialog.vue` (mirrors `DeleteOrgConfirmDialog.vue` minus the type-to-confirm input — deactivation is reversible) now gates unchecking Active; re-checking Active reactivates directly. `configOrg` is a computed lookup into `orgs.value` by id (not a captured snapshot) so the open drawer reflects post-`refreshOrgs()` state. `onToggleActive`/`onToggleAi`/`setOrgActive`/`setOrgAiEnabled` reused unchanged — client-only, no deploy. OrganizationsTab.test.ts 56/56 + DeactivateOrgConfirmDialog.test.ts 14/14; type-check clean; app suite at the 2-file baseline. | 2026-08-24 | d5db3eba, 738c2b85 | [260824-org-config-slideout](.planning/quick/260824-org-config-slideout/) |
| 260830-l9c | Two production auth defects. **(1) Multi-church login picker showed only 1 of N churches:** `loadOrgContext` built `memberships` solely from `users/{uid}.orgIds`, which the invite-accept path (`ensureUserDocument`) **replaced** with a single element (`orgIds: [inviteOrgId]`) instead of appending. Fixed with `arrayUnion(inviteOrgId)` (preserves `orgIds[0]` primary), AND self-healed the picker by sourcing its id set from the **union of the authoritative `claims.orgs` map ∪ `orgIds`** (drives both the memberships list and the auto-enter-vs-picker decision) — so already-clobbered accounts recover with **no manual Firestore repair**. **(2) Sign-out logged `permission-denied` / "Uncaught Error in snapshot listener":** `logout()` revoked the token before tearing down ~11 org-scoped store listeners. Fixed by calling `resetOrgScopedStores()` before `signOut(auth)`, plus a shared `src/utils/firestoreListener.ts` (`ignorePermissionDenied`/`isPermissionDenied`) wired into the logout-surviving component listeners (TeamView members+invites, GettingStarted) and a quiet-guard on appConfig + ConfigurationTab. type-check clean; auth.test.ts 104/104 + firestoreListener.test.ts 7/7; app suite at the 1-file storage.rules baseline. **⚠ NOT DEPLOYED** — needs `firebase deploy --only hosting` (client-only; the self-heal fixes the owner's own 2-church account once live). | 2026-08-30 | 94488be5, 33f4e18f, 338f550c, b4611708 | [260830-l9c-fix-multi-church-login-picker-orgids-arr](.planning/quick/260830-l9c-fix-multi-church-login-picker-orgids-arr/) |
| 260901-lua | **MAJOR data/availability fix.** The Phase 104 in-place sidebar "Switch Church" changed `authStore.orgId` with no route change/remount, so views that subscribed to their org-scoped stores only in `onMounted()` never re-subscribed to the new church — **Services stuck on "Loading services…"** (and Songs/Dashboard/Schedule/Volunteers showed stale/blank). An incompletely-applied Phase 104 fix: 104-REVIEW CR-01 fixed this for `TeamView`/`SettingsView` but missed the rest. Applied the same canonical `watch(() => authStore.orgId, cb, { immediate: true })` re-subscribe pattern to `ServicesView`/`SongsView`/`DashboardView`/`RosterView`/`QuarterView` (teardown → clear stale state → re-subscribe with the **live new** org id), and a fail-safe `ServiceEditorView` nav-to-`/services` on switch (its serviceId is org-specific). **Data-safety verified:** every re-subscribe uses the live new org id (no mount-time capture), zero `src/stores/` changes (write actions keep their `orgId.value` guard), so no cross-church write path. Validate mode; type-check clean; regression tests added; app suite at the documented 2-file baseline (`storage.rules.test.ts`, stale `appConfig.test.ts`). Client-only; **⚠ NOT DEPLOYED** — needs `firebase deploy --only hosting`. | 2026-09-01 | 6e7bf4c5, 8afc8288, c23cf412 | [260901-lua-fix-church-quick-select-menu-switching-c](.planning/quick/260901-lua-fix-church-quick-select-menu-switching-c/) |
| 260903-dismissible-monitor-warnings | Run screen: the "Finish setting up your displays" fallback help got stuck in the top band with no dismiss. Relocated it into `RunDisplaysPanel` (State B, where `outputStatus==='fallback'` is live) as a dismissible in-panel notice (`run-fallback-banner` kept + `/monitor-setup` link + `run-fallback-dismiss`, auto-resets on leaving fallback); the State-A `blocked`/`partial` pop-up-blocker banners gained a dismiss X (`run-blocked-dismiss`/`run-partial-dismiss`) via a `dismissedStatus` ref that resets on any `outputStatus` change (they can't move into the Displays panel — it isn't rendered in State A). +4 tests; RunControlView suites 81/81; type-check clean; full suite at the storage.rules-only baseline. Client-only; **⚠ NOT DEPLOYED** — ships with the next `firebase deploy --only hosting`. | 2026-09-03 | 97a5289f | [260903-dismissible-monitor-warnings](.planning/quick/260903-dismissible-monitor-warnings/) |
| 20260905-slideout-close-button-only | Song editing slideout (`SongSlideOver`) no longer closes on an outside/backdrop click — it now closes ONLY via the Close button or the X (both call `onCancel` → unsaved-guard → `emit('close')`). Removed `@click="onCancel"` from the backdrop; Escape was never wired here (confirmed by grep), so no Escape change needed. Prompted by the new Files tab making accidental closure costly. +2 tests (backdrop click emits no close; X still closes); SongSlideOver suite 34/34; type-check clean. Client-only; **⚠ NOT DEPLOYED** — ships with the next `firebase deploy --only hosting`. | 2026-09-05 | 759cead2 | [20260905-slideout-close-button-only](.planning/quick/20260905-slideout-close-button-only/) |
| 260908-dgq | "Messaging is off" notice on the Service **Messages tab**. The tab was hidden entirely when org messaging was off, leaving an editor unaware the feature exists. Now the tab always shows **for editors**; when messaging is off the panel shows "Messaging is off. **Configure messaging** so your team gets service plan updates." with a `router-link` to `{ name: 'settings' }`. Tab button + `visibleTabOrder` editor-gated only (dropped `isMessagingEnabled()`); new `messaging-off-notice` card (`isEditor && !isMessagingEnabled()`, mirrors the pc-credentials-missing-note idiom); `messaging-defaults-panel` now `v-if="isMessagingEnabled()"` so the notice replaces it when off; `service-message-history`'s kill-switch gate unchanged. Tests: rewrote the OFF-hides-tab case → now shows tab + notice + hides defaults, added ON inverse, and the "Messaging defaults panel (58-05, R132)" describe now runs with messaging enabled (its subject). ServiceEditorView 348/348; type-check clean; full suite at baseline. Client-only; **⚠ NOT DEPLOYED** — ships with next `firebase deploy --only hosting`. | 2026-09-08 | b2847586 | [260908-dgq](.planning/quick/260908-dgq/) |
| 260908-owm | **Sidebar divider + grouped, gridded Services Roles tab.** AppSidebar: a divider separates the volunteer-facing **My Schedule** from **Dashboard** and the editor/admin surfaces below (`separatorBefore` on Dashboard). Services → Roles tab: role cards are no longer one full-width column — a **responsive grid** (stack mobile, 2-up sm, 3-up lg) **grouped by role group** with a divider header per group (**Band / Tech / Other**). New `roleAssignmentGroups` computed; empty groups drop; card internals + testids unchanged. type-check clean; ServiceEditorView + AppSidebar 380/380. Client-only; **⚠ NOT DEPLOYED** — ships with next `firebase deploy --only hosting`. | 2026-09-08 | 0e708bfc | [260908-owm](.planning/quick/260908-owm/) |
| 260908-nq5 | **Confirm/Decline on My Schedule + a Decline state.** Moved the volunteer confirmation control off the service page onto My Schedule; added **Decline** as a distinct state leaders see on the dashboard. `ConfirmationStatus` gains `'declined'`; `firestore.rules` volunteer-arm pin relaxed to `status in ['confirmed','declined']` (needsReconfirmation stays editor-only; +rules tests, emulator 17/17). `VolunteerConfirmBar` reworked to a compact Confirm/Decline pair (whole-service worst-of; Decline writes `status:'declined'`/`confirmedAt:null`; click-active undoes). `ScheduleServiceCard` **de-linked** — the card is no longer a whole-surface link; "Rehearse →" is a real `router-link` button (`data-testid=rehearse-link`) with Confirm/Decline to its LEFT (Zone 3 `sm:w-auto`+`sm:flex-nowrap`, no wrap). Dashboard: red **Declined** chip; `unconfirmedAssignments` surfaces declined rows (each carries `status`); unconfirmed rows now identify the service by **full date** ("Sunday, September 7"), not the sometimes-blank name. Bar removed from `VolunteerServiceView`. type-check clean; targeted suites green. Client + **rules** change; **⚠ NOT DEPLOYED** — needs `firebase deploy --only hosting,firestore:rules`. | 2026-09-08 | 812c4da7 | [260908-nq5](.planning/quick/260908-nq5/) |
| 260909-scripture-selector-lock-fixes | **Bible-API-off UX + Mark-as-Planned lock races** (owner feature + live console errors). **A (requested):** AI scripture suggestions show an "Open in BibleGateway" deep-link when the org Bible API is off (`ScriptureInput.vue`, `aiSuggestionBibleGatewayLink`); hidden when on. **B (requested):** per-item ESV/NLT `<select>` (`ServiceEditorView.vue` `#version` slot) now `v-if="authStore.isBibleApiEnabled"` — inert when the API is off. **Thread 2:** `ServiceLockedError` on Mark-as-Planned (owner: exported→reopen→plan) was a regression from R420 (260908-cou) seeding the stage layout at lock — the seed mutates `localService` but the autosave watcher arms `'pending'` async, so `flush()`'s "nothing pending" check no-op'd and the seed's ~800ms debounce fired AFTER the lock (rejected, seed lost). Fix: `await nextTick()` between seed and `flush()`; CR-01 test updated (a full-doc seed-flush write may ride along; ME-02 re-expressed as "slots unchanged"). **Thread 3.3:** `[useSlideshowAssembly] group materialization write failed: permission-denied` — a fire-and-forget draft group write landing just after the service locks (`/slideGroups` rule rejects); expected + non-fatal (re-materializes next draft edit). Quieted to `console.warn` via `isPermissionDenied` (both materialization + rebuild catches); genuine failures still error (+1 test). **No-code triage:** BloomFilterError = benign Firestore SDK noise; `reportAllChanges`/`startTime` = browser-extension web-vitals (not in app code). type-check clean; ScriptureInput/ServiceEditorView/useSlideshowAssembly suites green; full app suite at storage.rules-only baseline. Client-only. ✅ **DEPLOYED to production (hosting) 2026-09-09**; tagged **v2.13.1**. | 2026-09-09 | 2bd7e625, d3cd92da, e0877b4e | [260909-scripture-selector-lock-fixes](.planning/quick/260909-scripture-selector-lock-fixes/) |
| 260909-ai-multichurch-gate | **AI/Bible enablement gate now follows the ACTIVE church (multi-church fix).** A multi-church owner got `403 "AI features are disabled for your organization"` on "suggest scripture" for a non-primary church despite AI being enabled there. **Root cause:** the server gate (`checkOrgAiEnablement`/`checkOrgBibleEnablement`) resolved the org from the login token's **primary `orgId` claim** (`orgIds[0]`, set at sign-in); the in-app church switcher changes `authStore.orgId` **without a new login**, so the claim never follows the switch — client gates on the viewed church (shows the button), server gates on the primary church (403). NOT caused by 260908-pca; the AI gate predates it (v2.2/Phase 82). **Fix — server** (`functions/src/index.ts`): new exported `resolveActiveOrgId(decoded, headerOrgId)` honors a client `X-Org-Id` header ONLY when the **verified token** proves the caller may act there — member (the `orgs` claim map has the key) or super-admin (`superAdmin === true`) — else falls back to the primary `orgId` claim; both proxy branches (anthropic; esv/nlt) resolve through it. Per-**uid** rate limiter untouched (quota attribution unchanged). **Fix — client** (`src/utils/appAuth.ts`): `getAppAuthHeaders()` sends `X-Org-Id = authStore.orgId`, try/catch-guarded (omit → server falls back to primary). **Fail-safe & backward-compatible:** missing/invalid header = prior behavior; can never widen access for a non-member. Frontend type-check clean, `appAuth.test.ts` 4/4, PC+claude green; functions build clean, full suite **721/721** (+7 `resolveActiveOrgId`). ✅ **DEPLOYED to production 2026-09-09** — `firebase deploy --only functions:api,hosting` (owner blanket deploy); `api` updated + hosting released. Owner test: switch to the AI-enabled church → "suggest scripture" should pass. | 2026-09-09 | 3af18088, f942b7b2 | [260909-ai-multichurch-gate](.planning/quick/260909-ai-multichurch-gate/) |
| 260908-pca | **Planning Center 401 in production** (worked in dev). Save/validate/export/import all failed with 401; the "credentials are empty" symptom followed because **Save & Validate** (`validatePcCredentials`) got the 401, reported "Invalid credentials", and never persisted. **Root cause:** `functions/src/index.ts` lists `planningcenter` in `AUTH_REQUIRED` (R339/SEC-A-01, Phase 117); the deployed Cloud Function proxy rejects any request lacking a valid `X-App-Auth` Firebase ID token with **401 "Authentication required"** BEFORE forwarding to PC. Every PC fetch sent only `Authorization: Basic <clientId:secret>` + `Accept`, never `X-App-Auth` — dev worked only because the Vite proxy has no guard; the AI/Bible calls were fine because they already attach it via `getAppAuthHeaders()`. **Fix #1:** added `...(await getAppAuthHeaders())` to **25** PC proxy fetch sites (24 `planningCenterApi.ts` + 1 `pcSongImport.ts`); Basic `Authorization` untouched (proxy strips `X-App-Auth` before forwarding); tests mock `getAppAuthHeaders`. **Fix #2 (cosmetic):** `SettingsView.vue` — PC token link → `api.planningcenteronline.com/personal_access_tokens`, "App ID" label/placeholder → "Client Id". type-check clean; planningCenterApi+pcSongImport+SettingsView 213/213. Client-only (hosting); no functions change. ✅ **DEPLOYED to production (hosting + firestore:rules) 2026-09-08** (owner granted blanket deploy); this hosting release also shipped the queued frontend backlog (260908-dgq/-owm/-nq5/-cou, 260903-dismissible-monitor-warnings, 20260905-slideout-close-button-only, 260901-lua, 260830-l9c) and nq5's `firestore.rules` relaxation is now live. **DEFERRED — AI 403:** `checkOrgAiEnablement` gates on the **token-claim** org's `aiMasterEnabled` (`resolveOrgId` = `orgId` custom claim), not the viewed church; owner confirmed master toggle ON for same church yet still 403s → likely token/claim vs enabled-org mismatch; needs prod org-doc + claim inspection. | 2026-09-08 | 8a4b55f6, ea5b82bd | [260908-pca](.planning/quick/260908-pca/) |
| 260908-cou | Stage-layout chits now track Service-Order role assignments. **(1)** Unchecking a person from a Band role in the Roles tab removes their stage chit immediately (`onToggleOverridePerson` → new `reconcileStageForRoleChange`, Band-only, rides useAutoSave). **(2)** A band member who also sings folds into ONE instrument chit with "Player also sings" (`withVocal`) auto-checked — no separate Vocals chit — both at auto-seed (`foldPlayAndSing`) and live (check/uncheck the vocal role sets/clears `withVocal`). New pure helpers `isVocalRoleName` (`/vocal\|vox\|choir\|sing/i`, excludes "lead" so "Lead Guitar" isn't misread) + `foldPlayAndSing`; `autoPopulateMarkers` honors a per-assignment `withVocal`. Live add of an instrument role creates no new chit (seed owns placement). +util tests (37) + 4 stage integration tests; ServiceEditorView 346/346; type-check clean; full suite at `storage.rules.test.ts`-only baseline. **Follow-up fix (same day, owner UAT): full live mirror** — the first cut missed reset-to-schedule (no reconcile on `onResetRoleOverride`) and didn't ADD chits for newly-assigned members. New pure `reconcileBandMarkers` + `syncStageWithRoles()` (called from both role handlers) now drops stale chits, keeps assigned ones in place (updating withVocal), and adds newly-assigned members into free grid cells; only mirrors a non-empty layout (seed owns first creation). **2nd follow-up (`810693da`): repopulate an emptied layout** — the guard short-circuited on `existing.length===0`, so once removals emptied the stage, re-adding people never brought chits back; now gated on `existing.length===0 && !stageLayoutAutoSeeded` so a SEEDED layout keeps mirroring even after it was emptied (+regression test; stage 28/28). Client-only; **⚠ NOT DEPLOYED** — ships with next `firebase deploy --only hosting`. | 2026-09-08 | 3fb268d2, b441dc4d, 810693da | [260908-cou](.planning/quick/260908-cou/) |
