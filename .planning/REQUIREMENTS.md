...
- [x] **R299**: **When disabled**, the user can **manually enter the passage text** so it becomes the slide/reading content (works with **any** version), and the LLM congregational split continues to operate on that text (still subject to the independent AI gate). **Refined by owner direction 2026-08-31 (v2.6 Phase-103 follow-up):** the separate "Paste the passage text" textarea was removed. A plain scripture slide has **no text-entry affordance** when the API is off (only the BibleGateway look-up deep-link) — its slide is just the reference. A **congregational reading** is composed directly in the existing bottom reading/format textarea (which can be pasted into normally); AI-split runs on that text via `hasPassageToSplit`/`onAiSplit`'s fallback to the textarea content.
