...
- [ ] **R298**: **When disabled**, scripture selection and congregational-reading UI offer an **"Open in BibleGateway" deep-link** for the entered reference in the desired version (works with **any** version, not just ESV/NLT), reusing the existing BibleGateway link builder in `src/utils/scripture.ts`.
...
- [ ] **R300**: **When disabled for an org**, the **"Bible Translation" selector is hidden** in that org's Settings (`SettingsView.vue`), mirroring how the "AI Features" card hides when the AI master gate is off — there is no API-backed version list to configure when the API is off.
