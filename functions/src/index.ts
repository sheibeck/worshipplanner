...
// cleanupSweeps (R359, Phase 120): implementation lives in its own module so
// its testable handlers can be imported directly (mirrors orgProvisioning.ts).
// renderedPrefixFor is shared with requestPptxRenderHandler below, so it is
// imported here rather than duplicated. The four onSchedule wrappers are
// re-exported at the bottom of this file -- see the comment there.
import {
  renderedPrefixFor,
...
// --- cleanupSweeps (R359, Phase 120: the four scheduled Storage-retention
// sweeps). Implementation lives in ./cleanupSweeps so its testable handlers
