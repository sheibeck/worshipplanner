...
// See ADR-0059 (docs/adr/0059-slideactionmenu-vue-s-aria-menu-pattern-reused-verbatim-open.md)
...
async function handleSwitch(targetOrgId: string): Promise<void> {
  ...
  try {
    ...
    // See ADR-0060 (docs/adr/0060-push-only-arms-its-auto-dismiss-timer-when-opts-is-omitted-e.md)
...
const navItems = computed(() => {
  ...
  if (authStore.isEditor) {
    items.push({
      label: 'Dashboard',
      to: '/',
      // 260908-owm: divider between the volunteer-facing My Schedule and the
      // editor/admin surfaces that follow.
