    ...
    <div class="px-6 py-8 max-w-4xl">
      ...
      <div v-else-if="phase === 'granted'">
        ...
        <!-- R427 — the Video output's background setting. Shown only when a
             ...
             REVIEW-FIX CR-01: hoisted out of the B2/editable-grid v-if/v-else
             split so it renders in BOTH granted-view branches — a returning
             operator who lands on the "already configured" summary (B2) must
             be able to reach this without clicking "Reassign roles". -->
        <div v-if="hasVideoRoleAssigned" class="mt-4 rounded-lg bg-gray-900 border border-gray-800 p-4" data-testid="video-key-color-card">
          ...
          <!-- REVIEW-FIX IN-01: the output window reads this setting once at
               open and does not live-reload it (see FullscreenSlideOutput.vue). -->
          <p class="text-xs text-gray-500 mt-1">Applies the next time you open the Video output.</p>
